	...
	for elev := 0; elev < NumElevators; elev++{
		...
		if elevatorList[elev].Dir == Dir_Stop && order.Floor == elevatorList[elev].Floor { //Standing still and order is at the same floor
			...
			if order.Button == Btn_Up {
				cost = order.Floor - elevatorList[elev].Floor +1 //+ They want to go in the same direction as you are already going
			}else{
				cost = order.Floor - elevatorList[elev].Floor + 3 // They do not want to go in the same direction
			}
			...
			if order.Button == Btn_Down {
				cost = elevatorList[elev].Floor - order.Floor +1 // + They want to go in the same direction as you are already going
			}else{
				cost = elevatorList[elev].Floor - order.Floor + 3// They do not want to go in the same direction
			}
