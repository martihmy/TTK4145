...
type govChans struct {
	EngineTimeOut	chan Elevator
}
...
func OrderHandler(id int, btnPressChan chan ButtonEvent, newOrderChan chan ButtonEvent, lightUpdaterChan chan [NumElevators]Elevator,
	...
	for {
		...
		case updateOnLocalElev := <- elevatorChan:
			...
			if elevatorList[id].State == Undefined {
				for floor :=0 ; floor < NumFloors; floor++ {
					for btn :=Btn_Up; btn < Btn_Cab; btn++{
						if elevatorList[id].Queue[floor][btn] {
							fmt.Println("Found a true in elevator list")
							reAssignedOrder := ButtonEvent{floor,btn,id,true}
	//						elevatorList[id].Queue[floor][btn] = false //NEWLY ADDED
							go func(){newOrderChan <- reAssignedOrder}()
							reAssignedOrder.Finished = false
							reAssignedOrder.DesignatedID = costCalculation(reAssignedOrder, elevatorList,id,onlineElevators)
							fmt.Println("New best elevator is id:",reAssignedOrder.DesignatedID)
							go func(){orderUpdateChan <- reAssignedOrder}()
			...
			if id==0 {
				for floor :=0 ; floor < NumFloors; floor++ {
					for btn :=Btn_Up; btn < Btn_Cab; btn++ {
						fmt.Println("update on local elev",elevatorList[id].Queue[floor][btn])
				}
			}
		...
		case newLocalOrder := <- btnPressChan:
			if !onlineElevators[id] && elevatorList[id].State != Undefined {
			...
			} else if !(newLocalOrder.Button == Btn_Cab && elevatorList[id].State == Undefined){
