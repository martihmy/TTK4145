...
func costCalculation(order ButtonEvent, elevatorList [NumElevators]Elevator, id int, onlineElevators[NumElevators]bool) int{
	...
	for elev := 0; elev < NumElevators; elev++{
		cost := 1000
		if !onlineElevators[elev] || elevatorList[elev].State == Undefined {
			continue
		}
...
/*func setCompletionTimer(floor int, btn int, start bool) {
	if floor == 0 && btn == Btn_Up {
		if start == true {
			ZeroUp_timer.Reset(5*time.Second)
		} else {
			ZeroUp_timer.Stop()
		}
	}
