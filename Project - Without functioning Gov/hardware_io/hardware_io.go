...
func PollButtons(ButtonPress chan<- ButtonEvent) {
		...
		for f := 0; f < _numFloors; f++ {
			for b := ButtonType(0); b < 3; b++ {
				...
				if v != prev[f][b] && v != false {
					ButtonPress<- ButtonEvent{f, ButtonType(b)}
				}
				prev[f][b] = v
			}
		}
...
func PollFloorSensor(FloorArrival chan<- int) {
	prev := -1
	for {
		time.Sleep(_pollRate)
		v := getFloor()
...
func InitElev(FloorArrival <-chan int) int {
